...
/// Internal namespace for OTLP message encoders. Each encoder takes a public
/// OTLP value type and returns its protobuf-encoded `Bytes` (the inner wire
/// payload — caller wraps with tag+length when embedding).
enum Encoder {
    // MARK: - AnyValue (oneof)
